...
class base(object):
    ...
    def __init__(self, page_name, element_name):
        ...
        self.x = self.elements.get('x')
        self.y = self.elements.get('y')
    ...
    def click_location(self):
        if self.type == "location":
            ...
            get_log().error(f"click_location方法未找到{self.page_name}下的{self.element_name}元素")

    # def click_location(self):
    #     # 通过坐标点击元素
    #     if self.type == 'location':
    #         uiautomation.Click(self.x, self.y)
    #         return True
    #     else:
    #         get_log().error(f'无法操作{self.page_name}下的{self.element_name}元素，请封装对应{self.type}定位的方法')
